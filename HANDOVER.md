...
| Status | Ring 7c: **all four automated tests PASS** (`./stage7/test-7c.sh`, 104 s at item 18); **test 5 in progress — Wajira, on the HP, by `stage7/METAL.md`: steps 1 to 4 passed on 17 September 2026, step 5 (`? ping`) stopped at `ERR: nic transmit timed out`; the fourth boot's `e1k:` line read `tdh 0` — the descriptor never fetched**; item 18 sets the ich8lan hardware bits before TCTL; the stick built at item 18 is to be flashed and the day run again from step 5. Ring 7b: **all five tests PASS** (test 5 confirmed by Wajira, 15 September 2026). Ring 7a: **all five tests PASS** (test 5 confirmed by Wajira, 9 September 2026). Stages 0–5 and the three ring 6 gates green on their own binaries; both Stage 7 gates green on the 7b binary. |
...
instructions fit in its padding); the stick 69,206,016 bytes (135,168
...
boot and the fourth boot's `e1k:` line read `tdh 0` (item 17); item 18
sets the ich8lan hardware bits before TCTL.** The stick built at item 18
...
the strip — item 18's bits were the fix; the day goes on to steps 6–8,
and the closure item writes the CLAUDE.md gotcha (the PCH's LAN fetches
no descriptor without the ich8lan hardware bits); or the echo, then the
`e1k:` line and `ERR: nic transmit timed out` again — read it by item 17's
rule, now with item 18's bits on the line: **`tdh 0` still, with `txdctl
0x00400000` and `tarc0` showing bits 23, 24, 26, 27 over `0x403`** (or
fewer, if the 82579LM reads some as 0 the way QEMU's model does — record
what it shows) — the bits are in and did not suffice: **the ME item
next** (`fwsm` FW_VALID set: the `EXTCNF_CTRL.SWFLAG` semaphore from item
16's neighbour (a), then the TXDCTL write-back policy and `CTRL_EXT.RO_DIS`
behind it, item 18's list); **`tdh 1`** — the frame went out and the `DD`
write-back is missing: the write-back item (`RS` honoured,
`TXDCTL0.WTHRESH`, the descriptor re-read); **`status` bit 4** set — flow
control has paused transmit: the flow-control item (`CTRL.RFCE/TFCE`, the
`FCT*` registers). The twin's own line (`tdh 1 tdt 1 … sta 0x01`) is the
reference for the shape. Then the chart and the photograph into
