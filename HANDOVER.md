...
| 5 | **Oracle — Wajira, on the HP** — the stick, the lines on the chart, the glass at native, a note across a power cycle, `? ping` over the LAN, `! install calculator` and its launch with the broker off, a click | **PENDING** — the first watched boot, 17 September 2026, stopped after `S7: home 0 apps` before the nic line (item 16: the 82579LM's reset timing); reflash the stick built at item 16 and again by `stage7/METAL.md` |
...
**Ring 7c is green pending the oracle; the first watched boot found item
16.** The stick built at item 16 (`stage7/out/stick.img`, the file as
built, 36,864-byte binary) must be **flashed again** by `stage7/METAL.md`
steps 1–3 — the owner's hand, the by-id path — then the day from step 5:
the LAN port's second address is already on the port if it was added on 17
September (`ip -4 addr show <port>` says), and the three terminals at the
repo root (the broker and the relay from the first day were stopped at
item 16 so the gates could run; the chart reader was left running and
must be restarted on a fresh log):
...
then power on (6), the first boot read line by line against the
debugging table (7). **What the next chart should show after `S7: home N
apps`:** `S7: nic 6c:3b:e5:3b:86:45` then `S7: link up` and the rest to
`S7: keyboard ready` — item 16 was the whole of it; or `S7: nic …` then
`ERR: nic link did not come up within 10 s` — the named error, the PCH
PHY, the next item with `ich8lan.c` and the datasheet open; or `S7: nic …`
with a wrong MAC or `ERR: nic has no address in RAL/RAH` — the `SWFLAG`
neighbour from item 16; or nothing again after `home` — another MMIO touch
the PCH would not take, between the reset and `msg_nic`. Then test 5's
eight steps (8), and the chart and the photograph into `history/` (9). His
word closes the ring and Stage 7; then the N-of-1 trials ring, per the
spec's order. The first day's chart is `stage7/out/metal.log` and the
HP's `lspci` from live Ubuntu is `stage7/out/hp-lspci.log`; both are
gitignored under `out/` and should be copied into `history/` as
`2026-09-17-ring7c-first-boot-serial.log` and
`2026-09-17-ring7c-hp-lspci.log` by the owner's hand if he wants them kept.
