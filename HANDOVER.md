...
| Status | Ring 7c: **all four automated tests PASS** (`./stage7/test-7c.sh`, 104 s at item 19); **test 5 in progress — Wajira, on the HP, by `stage7/METAL.md`: steps 1 to 4 passed on 17 September 2026, step 5 (`? ping`) stopped at `ERR: nic transmit timed out`; the fourth and fifth boots' `e1k:` lines read `tdh 0` — the descriptor never fetched, with item 18's hardware bits in**; item 19 waits for the ME's window before every register write, reads the ring registers back, and puts the device's TDLEN/TDBAL/TDBAH on the line; the stick built at item 19 is to be flashed and the day run again from step 5. Ring 7b: **all five tests PASS** (test 5 confirmed by Wajira, 15 September 2026). Ring 7a: **all five tests PASS** (test 5 confirmed by Wajira, 9 September 2026). Stages 0–5 and the three ring 6 gates green on their own binaries; both Stage 7 gates green on the 7b binary. |
...
instructions and item 19's three routines, six read-backs and twenty-nine
call sites fit in its padding); the stick 69,206,016 bytes (135,168
...
boot, the fourth and fifth boots' `e1k:` lines read `tdh 0` (item 17)
with item 18's bits in on the fifth; item 19 waits on the ME's window
before every register write, reads the ring registers back and puts the
device's TDLEN/TDBAL/TDBAH on the line.** The stick built at item 19
(`stage7/out/stick.img`, the file as built, 40,960-byte binary) must be
**flashed again** by `stage7/METAL.md` steps 1–3 — the owner's hand, the
by-id path — then the day from step 5 of section 5: the LAN port's second
address is already on the port if it was added on 17 September (`ip -4
addr show <port>` says), and the three terminals at the repo root (the
broker and the relay were stopped so the gates could run; the chart
reader must be restarted on a fresh log):
...
the note back above the prompt. Then **test 5 from step 5** (section 8):
`? ping`. **What the chart will say:** `pong` on the screen and `w 001` on
the strip — item 19 closes step 5 and earns the CLAUDE.md gotcha (the
82579 with the ME alive loses a register write issued inside the ME's
window; wait on FWSM bit 24 and read the ring registers back); the day
goes on to steps 6–8. Or the echo, then the widened `e1k:` line and `ERR:
nic transmit timed out` again — read it by item 17's rule with item 19's
three fields: **`tdh 0` still with `tdlen 128` and `tdbal` equal to the
low half of `expect`** — the ring is configured in the device and the
fetch is gated elsewhere: item 18's list, the `EXTCNF_CTRL.SWFLAG`
semaphore first, then the TXDCTL write-back policy and `CTRL_EXT.RO_DIS`;
**`tdlen 0`, or a `tdbal`/`tdbah` that does not match `expect`** — a lost
write the protection did not catch: the window is not the one Linux waits
on, and the next item reads `e1000_reset_hw_ich8lan` whole for the ME's
state around `CTRL.RST`; **`ERR: nic register 0x… wrote 0x… read 0x… - it
will not hold its value`** before `S7: component region` — the ME
overwrites what the host writes, the rewrite bound (8) is the next number
to read; **`tdh 1`** — the frame went out and the `DD` write-back is
missing: the write-back item (`RS` honoured, `TXDCTL0.WTHRESH`, the
descriptor re-read); **`status` bit 4** set — flow control has paused
transmit: the flow-control item (`CTRL.RFCE/TFCE`, the `FCT*` registers).
The twin's own line (`tdh 1 tdt 1 … sta 0x01 tdlen 128 tdbal 0x004ce000
tdbah 0x00000000 expect 0x00000000004ce000`) is the reference for the
shape. Then the chart and the photograph into
`history/` (9). His word closes the ring and
