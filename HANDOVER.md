...
| Status | Ring 7c: **all four automated tests PASS** (`./stage7/test-7c.sh`, 104 s at item 17); **test 5 in progress — Wajira, on the HP, by `stage7/METAL.md`: steps 1 to 4 passed on 17 September 2026, step 5 (`? ping`) stopped at `ERR: nic transmit timed out`**; item 17 puts the device's state on the chart before that halt; the stick built at item 17 is to be flashed and the day run again from step 5. Ring 7b: **all five tests PASS** (test 5 confirmed by Wajira, 15 September 2026). Ring 7a: **all five tests PASS** (test 5 confirmed by Wajira, 9 September 2026). Stages 0–5 and the three ring 6 gates green on their own binaries; both Stage 7 gates green on the 7b binary. |
...
September 2026 and step 5 stopped at the transmit timeout (item 17).** The
stick built at item 17 (`stage7/out/stick.img`, the file as built,
40,960-byte binary) must be **flashed again** by `stage7/METAL.md` steps
1–3 — the owner's hand, the by-id path — then the day from step 5 of
section 5: the LAN port's second address is already on the port if it was
added on 17 September (`ip -4 addr show <port>` says), and the three
terminals at the repo root (the broker and the relay were stopped so the
gates could run; the chart reader must be restarted on a fresh log):
...
the strip — the timeout was transient and the day goes on to steps 6–8;
or the echo, then the `e1k:` line and `ERR: nic transmit timed out` — read
the line by item 17's rule and open the next item on it: **`tdh 0`** — the
descriptor was never fetched: the descriptor-fetch item, the `TXDCTL0` and
`TARC0` bits `e1000_initialize_hw_bits_ich8lan` sets before a transmit
(`ich8lan.c`), one bounded change per boot; **`tdh 1`** — the frame went
out and the `DD` write-back is missing: the write-back item (`RS` honoured,
...
`FCT*` registers); **`fwsm`** non-zero in its semaphore bits — the ME holds
the interface: the `SWFLAG` neighbour from item 16, first. The twin's own
line (`tdh 1 tdt 1 … sta 0x01`) is the reference for the shape. Then the
chart and the photograph into `history/` (9). His word closes the ring and
