...
September 2026; step 5 stopped at the transmit timeout on six boots; the
seventh (18 September) found the cause through item 20's monitor — the
absolute TCTL write cleared the 82579LM's `MULR` bit — and item 21 is the
fix, proven on the HP itself through the monitor before it was written
(TCTL `0x3003f0fa`, TARC1 `0x45000403`, `tdh 2 tdt 2 sta 0x01`).** The
stick built at item 21 (`stage7/out/stick.img`, the file as built,
40,960-byte binary) must be **flashed once** by `stage7/METAL.md` steps
1–3 — the owner's hand, the by-id path — after the HP is powered off (it
was left up in the monitor; Ctrl-C ends the probe with the log complete).
Then the day again from step 5 of section 5: the LAN port's second
address is already on the port (`ip -4 addr show <port>` says), and the
three terminals at the repo root, **the probe in place of the chart** so
the monitor is there if it is needed (the broker and the relay were
stopped so the gates could run):
...
**Expected: `pong` on the screen and `w 001` on the strip** — then the
day goes on to steps 6–8 (`! install calculator`, its launch with the
broker off, a click). **If instead the `e1k:` line comes again**, read
its `tctl` field first (`0x3003f0fa` means the fix reached the device and
the cause is elsewhere; anything else means it did not), type nothing
more on the HP, and a CC session asks through `127.0.0.1:9995` as on the
18th (`stage7/out/probe7c/monclient.py` is the client; item 21 records
what has already been asked and answered). Then the chart and the
photograph into `history/` (9). His word closes the ring and Stage 7;
then the N-of-1 trials ring, per the spec's order. The day's chart is
`stage7/out/metal.log` (every boot, and the monitor's dialogue of 18
September at lines 128 to 3314) and the HP's `lspci` from live Ubuntu is
`stage7/out/hp-lspci.log`; both are gitignored under `out/` and should be
copied into `history/` as `2026-09-18-ring7c-serial.log` and
`2026-09-17-ring7c-hp-lspci.log` by the owner's hand if he wants them
kept.
