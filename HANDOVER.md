...
| Status | Ring 7c: **all four automated tests PASS** (`./stage7/test-7c.sh`, 104 s at item 19); **test 5 in progress — Wajira, on the HP, by `stage7/METAL.md`: steps 1 to 4 passed on 17 September 2026, step 5 (`? ping`) stopped at `ERR: nic transmit timed out`; the fourth, fifth and sixth boots' `e1k:` lines read `tdh 0` — the descriptor never fetched, with item 18's hardware bits in and, on the sixth, item 19's read-back showing the ring configured in the device as written (`tdlen 128`, `tdbal` the low half of `expect`)**; item 20 is the serial monitor — on the transmit timeout the guest answers `r`, `w`, `d`, `t`, `m`, `q` over COM1 instead of halting, and `broker/probe.py` on mlrig teeing the port to a log and to one socket client on `127.0.0.1:9995`, so the next questions cost no flash; the stick built at item 20 is to be flashed once and the investigation driven over the socket. Ring 7b: **all five tests PASS** (test 5 confirmed by Wajira, 15 September 2026). Ring 7a: **all five tests PASS** (test 5 confirmed by Wajira, 9 September 2026). Stages 0–5 and the three ring 6 gates green on their own binaries; both Stage 7 gates green on the 7b binary. |
...
| 5 | **Oracle — Wajira, on the HP** — the stick, the lines on the chart, the glass at native, a note across a power cycle, `? ping` over the LAN, `! install calculator` and its launch with the broker off, a click | **IN PROGRESS — steps 1 to 4 passed on 17 September 2026** (the second and third watched boots: eighteen lines in the twin's order, the glass at 1920x1080, `hello metal` kept across a power cycle — the photographs in `history/`); **step 5 stopped at `ERR: nic transmit timed out`** (the first frame's `DD` never set on the 82579LM; item 17 names the device's state before that halt; **the fourth boot's line read `tdh 0`, `txdctl 0`, `tarc0 0x403`, FWSM FW_VALID — the descriptor never fetched; item 18 sets the ich8lan hardware bits; the fifth boot's line read `tdh 0` still with `txdctl 0x00400000` and `tarc0 0x0d800403` — the bits in and not enough; item 19 waits on the ME's window before every register write, reads the ring registers back and widens the line with the device's TDLEN/TDBAL/TDBAH; **the sixth boot's line read `tdh 0` with `tdlen 128 tdbal 0x004ce000 tdbah 0x00000000 expect 0x00000000004ce000` — the ring configured in the device exactly as written, the lost-write branch closed; item 20 is the serial monitor and `broker/probe.py`, so the questions from here are asked over the wire, many per boot, no flash between them**). The first watched boot stopped after `S7: home 0 apps` before the nic line (item 16). Flash the stick built at item 20 once, boot with probe.py in place of the chart, type `? ping`; then a CC session drives the monitor through `127.0.0.1:9995` (`stage7/METAL.md`, "The monitor") |

**On this build** (15 September 2026; the sizes as of item 20, 17
...
call sites, and item 20's monitor — five routines, twelve defines, ten
strings and the register table — fit in its padding); the stick 69,206,016 bytes (135,168
...
**Ring 7c is green pending the oracle; test 5 steps 1 to 4 passed on 17
September 2026, step 5 stopped at the transmit timeout on the second
boot; the fourth, fifth and sixth boots' `e1k:` lines read `tdh 0` with
every register we can name reading right (item 19's read-back on the
sixth); item 20 is the serial monitor, so the next questions are asked
over the wire, many per boot, with no flash between them.** The stick
built at item 20 (`stage7/out/stick.img`, the file as built, 40,960-byte
binary) must be **flashed once more** by `stage7/METAL.md` steps 1–3 —
the owner's hand, the by-id path — then the day from step 5 of section 5:
the LAN port's second address is already on the port if it was added on
17 September (`ip -4 addr show <port>` says), and the three terminals at
the repo root, **the probe in place of the chart** (the broker and the
relay were stopped so the gates could run):
...
then power on (6): eighteen lines to `S7: keyboard ready` as on the 17th,
the note back above the prompt. Then `? ping` (section 8, step 5).
**What the probe's terminal will say:** the echo, the widened `e1k:` line,
`ERR: nic transmit timed out`, then **`mon: ready`** — the machine stays
up, the glass keeps painting, and the monitor waits for its first line.
(Or `pong` on the screen and `w 001` on the strip — then nothing was
wrong that a sixth flash did not fix, and the day goes on to steps 6–8.)
**From `mon: ready` the owner types nothing more on the HP**: a fresh CC
session drives the monitor through `127.0.0.1:9995` on mlrig (`nc
127.0.0.1 9995` shows the dialogue; a Python client is the session's
tool), by `stage7/METAL.md` "The monitor" and item 20's table of commands,
in the order item 20 lists Linux's steps: `d` first (the whole state,
PBA, WUC, KABGTXD, FEXTNVM3, EXTCNF_CTRL among it), then STATUS bits 9
and 10 (`r 8`; `w 8 <value with bit 10 clear>`), then the PHY reset with
`RST` through `w 0` (the monitor waits 25 ms before the read-back),
`LAN_INIT_DONE` awaited by `r 8`, `PHYRA` cleared, the ring registers
re-checked by `d`, and `t` after each step to see whether the head moves
(`mon: t tdh 2 …` with `sta 0x01` is the fix found). Every register read
and written is on the log with a timestamp; the log is the brief for the
fix item, which is one item, one flash, and earns the CLAUDE.md gotcha.
The twin's own `d` line (`tdh 1 tdt 1 … sta 0x01 tdlen 128 … rdlen 256
rdh 1 rdt 15 rxdctl 0x00010000 gcr 0x06800200`, item 20) is the reference
for the shape. Then the chart and the photograph into `history/` (9). His word closes the ring and
Stage 7; then the N-of-1 trials ring, per the spec's order. The day's
chart is `stage7/out/metal.log` (all three boots) and the HP's `lspci`
from live Ubuntu is `stage7/out/hp-lspci.log`; both are gitignored under
`out/` and should be copied into `history/` as
`2026-09-17-ring7c-serial.log` and `2026-09-17-ring7c-hp-lspci.log` by
the owner's hand if he wants them kept.
