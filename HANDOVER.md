...
| 5 | **Oracle — Wajira, on the HP** — the stick, the lines on the chart, the glass at native, a note across a power cycle, `? ping` over the LAN, `! install calculator` and its launch with the broker off, a click | **PENDING** — `stage7/METAL.md` |
...
in this order: **Cowork's pre-oracle review** of the guest diff (items 7,
8, 9 in `stage7/stage7.asm`), `stage7/mkstick.py`, `broker/relay.py`'s
grace close, `broker/chart.py` and `stage7/METAL.md`; then **the owner's
day by `stage7/METAL.md`**, step by step — the flash by his own hand by
the by-id path (steps 1–3), the wiring (4), the LAN port's second address
and the three terminals at the repo root (5):
...
debugging table (7), test 5's eight steps (8), and the chart and the
photograph into `history/` (9). His word closes the ring and Stage 7;
then the N-of-1 trials ring, per the spec's order.
