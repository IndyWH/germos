...
| | |
|---|---|
| Stage | **7 — Metal — CLOSED 18 September 2026** (opened 9 September): ring 7a (the disk) **closed 9 September**; ring 7b (the wire) **closed 15 September**; **ring 7c (the metal) CLOSED 18 September 2026** by the owner's word after test 5 on the HP, all eight steps (opened 15 September) — next is the N-of-1 trials ring, per `stage7/spec.md`'s order — `stage7/plan-7c.md` approved with Cowork's seven amendments, medium effort by the spec. Stages 0–6 closed (Stage 6 on 5 September 2026) |
| Status | **Ring 7c: all five tests PASS — test 5 confirmed by Wajira on the HP, 18 September 2026, all eight steps; his word closed the ring and Stage 7** (the eighth and ninth watched boots, the item 21 binary: `pong` with `w 001`, `installed calculator` with `g 000/001` and `w 002`, then with the broker off `S7: home 1 apps`, the arrow, and the calculator launched from the disk by a click — 4096, `w 000`, `io 000000/000000`; the photographs and the serial log in `history/`; the three gates green at the closure). The history of the ring's test 5, kept: Ring 7c: **all four automated tests PASS** (`./stage7/test-7c.sh`, 104 s at item 19); **test 5 was in progress — Wajira, on the HP, by `stage7/METAL.md`: steps 1 to 4 passed on 17 September 2026, step 5 (`? ping`) stopped at `ERR: nic transmit timed out`; the fourth, fifth and sixth boots' `e1k:` lines read `tdh 0` — the descriptor never fetched, with item 18's hardware bits in and, on the sixth, item 19's read-back showing the ring configured in the device as written (`tdlen 128`, `tdbal` the low half of `expect`)**; item 20 is the serial monitor — on the transmit timeout the guest answers `r`, `w`, `d`, `t`, `m`, `q` over COM1 instead of halting, and `broker/probe.py` on mlrig teeing the port to a log and to one socket client on `127.0.0.1:9995`, so the next questions cost no flash; **the seventh boot (18 September 2026) found the cause through that monitor in one boot — the absolute TCTL write cleared the 82579LM's `MULR` (bit 28), and with it clear the MAC never commits a packet with `EOP` to the transmitter; item 21 writes TCTL read-modify-write with `MULR` and clears TARC1 bit 28, the configuration proven on the HP through the monitor (`tdh 2 tdt 2 sta 0x01`); the three gates green at item 21; the stick built at item 21 is to be flashed once and the day run again from step 5, expecting `pong`**. Ring 7b: **all five tests PASS** (test 5 confirmed by Wajira, 15 September 2026). Ring 7a: **all five tests PASS** (test 5 confirmed by Wajira, 9 September 2026). Stages 0–5 and the three ring 6 gates green on their own binaries; both Stage 7 gates green on the 7b binary. |
...
| Model | **Ring 7c, as it closed: CC ran every item (0 to 21 and the closure) on Fable 5.1 at medium effort; Cowork was Fable 5.1 except for a stretch during the item 19 diagnosis, when its session fell back to Opus 4.8 and the owner briefly set Opus 5 at high effort, then back to Fable 5.1 — the DRV_LOAD misstep and its retraction happened in that stretch, the PCIM2PCI finding under Opus 5, confirmed under Fable; the item 21 diagnosis was CC on Fable 5.1 at medium effort through the monitor in one boot.** **Ring 7c: Fable 5.1 at medium effort** — the owner's decision in the spec, recorded at item 0 (the spec sets high for 7a and 7b, medium for 7c, decided at each gate); to be compared with ring 6b, the earlier medium-effort ring. Ring 7b was Fable 5.1 at high effort — one session, thirteen commits plus 10b, one freeze opening of ring 6b item 10b's class, zero review defects, zero oracle defects. Ring 7a was Fable 5.1 at high effort. Ring 6c was Fable 5.1 at high; ring 6b Fable 5.1 at medium, the owner's experiment, to be compared with Opus at high (Stages 0 and 1) and Fable at high (Stages 2 to 6a, 6c); Cowork reviews to the same standard as every ring |
...
**Ring 7c and Stage 7 are CLOSED — 18 September 2026, by the owner's
