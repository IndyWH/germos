#!/usr/bin/env bash
#
# Stage 4 build and pack.
#
# Assembles stage4/stage4.asm into a PE32+ UEFI application and packs it into a
# FAT image at the UEFI removable-media path, EFI/BOOT/BOOTX64.EFI, where OVMF
# looks for it. Stage 3's proven recipe, retargeted.
#
# This script is DELIBERATELY NOT FROZEN, unlike stage4/test.sh,
# stage4/checkumbilical.py and stage4/UMBILICAL.md. It is the recipe, not a
...
#
# Everything is written inside stage4/out/. No block device is touched, no loop
# mount is taken, and dd writes only to a file in that directory.
#
# Usage:  ./stage4/mkimage.sh        (from anywhere)
...
if [ ! -f "$ASM" ]; then
  echo "mkimage: stage4/stage4.asm does not exist yet - nothing to build" >&2
  exit 1
fi

# Assembled from the repo root: stage4.asm incbins the font by its
