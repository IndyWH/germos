#!/usr/bin/env bash
#
# Stage 5 build and pack.
#
# Assembles stage5/stage5.asm into a PE32+ UEFI application and packs it into a
# FAT image at the UEFI removable-media path, EFI/BOOT/BOOTX64.EFI, where OVMF
# looks for it. Stage 4's proven recipe, retargeted.
#
# This script is DELIBERATELY NOT FROZEN, unlike stage5/test.sh,
# stage5/checkgermline.py and stage5/GERMLINE.md. It is the recipe, not a
...
#
# Everything is written inside stage5/out/. No block device is touched, no loop
# mount is taken, and dd writes only to a file in that directory. The same
# image is what the broker's rehearsal boots as the twin.
#
# Usage:  ./stage5/mkimage.sh        (from anywhere)
...
if [ ! -f "$ASM" ]; then
  echo "mkimage: stage5/stage5.asm does not exist yet - nothing to build" >&2
  exit 1
fi

# Assembled from the repo root: stage5.asm incbins the font by its
