...
;
; Grown on Stage 4's proven body: the same hand-assembled PE32+ UEFI
...
; interrupt-driven keyboard, the virtio-blk disk and the notebook, the
; virtio-net NIC with ARP, IPv4 and a client-only TCP, and the question per
; stage4/UMBILICAL.md - all kept. Across plan items 10 to 12 the machine
; learns to grow: a fixed component region in its own BSS (line twelve of
; the boot log says where), millisecond ticks from the calibrated TSC, the
; forgiving marker parse ("?" asks, "!" requests, with or without a space),
; the grow request as a marker-byte frame per stage5/GERMLINE.md, the
; binary component frame received straight into the region, and the loader
; - a call into the blob with one register at a four-entry service table
; (draw text at a cell, console size, poll a key, ticks), the screen cleared
; before and restored from the shadow after, Esc the way back to the prompt.
; The network is slirp's cage: restrict=on and one guestfwd, so the guest's
; whole world is the broker at 10.0.2.4:9999 - which rehearses every
; candidate in a headless boot of THIS image before delivering it.
;
...
; disk this code has ever seen is a raw file under stage5/out/. Plaintext
; inside the cage this ring; TLS lives in the broker.
;
; Built with:  nasm -f bin stage5/stage5.asm -o stage5/out/BOOTX64.EFI
...
.all_in:
        ...
        ; interrupts off and polled, so that S5: keyboard ready stays the
...
; records and log "S5: notebook <N> notes"; otherwise write the header and a
; zeroed sector 1 and log "S5: notebook formatted". Called once from
...
; Called once from efi_main after "S5: keyboard ready"; clobbers registers.
notebook_replay:
        mov     ebx, 1
.note:
        cmp     ebx, [nb_count]
...
; S5: lines the acceptance tests count, so a failure path can say what went
...
msg_alive:      db      'S5: alive', 13, 10, 0

msg_gop:        db      'S5: gop ', 0
msg_fb:         db      ' fb 0x', 0
msg_exited:     db      'S5: boot services exited', 13, 10, 0
msg_paging:     db      'S5: gdt and paging ours', 13, 10, 0
msg_idt:        db      'S5: idt ready', 13, 10, 0
msg_found:      db      'S5: cores found ', 0
msg_woken:      db      'S5: cores woken ', 0
msg_console:    db      'S5: console ', 0
msg_disk:       db      'S5: disk ', 0
msg_sectors:    db      ' sectors', 13, 10, 0
msg_nb:         db      'S5: notebook ', 0
