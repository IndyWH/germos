#!/usr/bin/env bash
#
# Stage 6 ring 6a build and pack.
#
# Assembles stage6/stage6.asm into a PE32+ UEFI application and packs it into a
# FAT image at the UEFI removable-media path, EFI/BOOT/BOOTX64.EFI, where OVMF
# looks for it. Stage 5's proven recipe, retargeted.
#
# This script is DELIBERATELY NOT FROZEN, unlike stage6/test.sh,
# stage6/checkglass.py and stage6/GLASS.md. It is the recipe, not a
...
#
# Everything is written inside stage6/out/. No block device is touched, no loop
# mount is taken, and dd writes only to a file in that directory. The same
# image is what the broker's twin boots, from its own private copy.
#
# Usage:  ./stage6/mkimage.sh        (from anywhere)
...
if [ ! -f "$ASM" ]; then
  echo "mkimage: stage6/stage6.asm does not exist yet - nothing to build" >&2
  exit 1
fi

# Assembled from the repo root: stage6.asm incbins the font by its
...
# mount, no partition table. Confirmed to boot under OVMF since Stage 1.
