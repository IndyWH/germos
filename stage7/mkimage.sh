#!/usr/bin/env bash
#
# Stage 7 ring 7a build and pack.
#
# Assembles stage7/stage7.asm into a PE32+ UEFI application and packs it into a
# FAT image at the UEFI removable-media path, EFI/BOOT/BOOTX64.EFI, where OVMF
# looks for it. Stage 6's proven recipe, retargeted.
#
# This script is DELIBERATELY NOT FROZEN, unlike stage7/test.sh,
# stage7/checkdisk.py and stage7/DISK.md. It is the recipe, not a
...
#
# Everything is written inside stage7/out/. No block device is touched, no loop
...
#
# Usage:  ./stage7/mkimage.sh        (from anywhere)
...
if [ ! -f "$ASM" ]; then
  echo "mkimage: stage7/stage7.asm does not exist yet - nothing to build" >&2
  exit 1
fi

# Assembled from the repo root: stage7.asm incbins the font by its
