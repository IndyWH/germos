...
#
# Assembles stage2/stage2.asm into a PE32+ UEFI application and packs it into a
# FAT image at the UEFI removable-media path, EFI/BOOT/BOOTX64.EFI, where OVMF
# looks for it. Stage 1's proven recipe, retargeted.
#
# This script is DELIBERATELY NOT FROZEN, unlike stage2/test.sh,
# stage2/checktext.py and stage2/font8x8.bin. It is the recipe, not a
...
#
# Everything is written inside stage2/out/. No block device is touched, no loop
# mount is taken, and dd writes only to a file in that directory.
#
# Usage:  ./stage2/mkimage.sh        (from anywhere)
...
if [ ! -f "$ASM" ]; then
  echo "mkimage: stage2/stage2.asm does not exist yet - nothing to build" >&2
  exit 1
fi

# Assembled from the repo root: stage2.asm incbins the font by its
