...
| **7 — Bare metal** | The same binary on a real machine: an HP Compaq Elite 8300 from 2012, booted from a stick the owner flashed by hand. An AHCI driver and a GPT the machine writes itself; an e1000e driver on the 82579LM with a relay answering on the home switch; the EDID guard; the i8042 initialised cold; serial first, so every device reports before the next is touched. The binary that booted is 36,864 bytes. The Ubuntu 26.04.1 image that boots the same machine from the same drive is 6,482,409,472 bytes, 175,841 times larger. From the firmware's handover to a prompt you can type at: 0.64 seconds, timestamped on the serial chart. Shutdown is the power button. | opened 9 Sep 2026 · booted on the metal 17 Sep 2026 · the wire on the metal still open | [the HP and its screen](history/2026-09-17-ring7c-first-glass-on-metal-1600.jpg) · [hello metal](history/2026-09-17-ring7c-hello-metal-1600.jpg) |
...
**The metal, 17 September 2026.** Every driver was rehearsed in a twin of the HP for eight days before the machine saw the binary. The first watched boot stopped twelve lines in, with no error and a black screen. A live Ubuntu on the same machine proved the NIC healthy, and Intel's own driver source named the cause: on the PCH's integrated LAN, a register read in the instruction after the reset write hangs the processor. One item, nine bytes, a 25 ms wait, and the next boot went all the way. Four cores woken, the 250 GB SATA disk found on port 0 carrying the table a blind boot had left two days earlier, the link up at a gigabit, the glass painted by core 2, `hello metal` typed and back after the power button: 0.64 seconds from the firmware's handover to `S7: keyboard ready`, in a binary of 36,864 bytes. Then `? ping` stopped at a named error, `ERR: nic transmit timed out`, and the boot order did its job: every register the driver writes has since been read back from the 82579LM and matches, and the next item puts a monitor on the serial line so the chip can be questioned over the wire, many times per boot, instead of one flash per question. The whole of it was written in assembly by Claude, rehearsed in QEMU before any of it ran on the HP, and flashed by the owner's hand, the one step the tools are forbidden to take. The originals of the two photographs, straight from the phone, are beside the web copies in `history/`.
