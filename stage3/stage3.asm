...
;
; Grown on Stage 2's proven body: the same hand-assembled PE32+ UEFI
...
; walk, every core woken with INIT-SIPI-SIPI, the text console and the
; interrupt-driven keyboard - all kept. Two organs grow across plan items 9 to
; 12: a virtio-blk driver found on the PCI bus and driven through the modern
; interface, and the notebook - an append-only journal of notes on that disk,
; per stage3/NOTEBOOK.md. Every line entered at the prompt is written through
; before the next prompt appears; the next boot replays it above the prompt.
;
...
; disk this code has ever seen is a raw file under stage3/out/.
;
; Built with:  nasm -f bin stage3/stage3.asm -o stage3/out/BOOTX64.EFI
...
; S3: lines the acceptance tests count, so a failure path can say what went
...
msg_alive:      db      'S3: alive', 13, 10, 0

msg_gop:        db      'S3: gop ', 0
msg_fb:         db      ' fb 0x', 0
msg_exited:     db      'S3: boot services exited', 13, 10, 0
msg_paging:     db      'S3: gdt and paging ours', 13, 10, 0
msg_idt:        db      'S3: idt ready', 13, 10, 0
